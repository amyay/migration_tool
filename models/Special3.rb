class Special3
  ...
  def export
    @logger.debug "Exporting tickets, comments, user and groups altogether ..."
    batch_size = 25
    # batch_size = 30
    filecount = 0
    count = 0
    ...
    Ticket.find_each do |t|
      ...
      next if !(t.id > 164750 && t.id < 164876)

      if (count % batch_size == 0)
        # open files #
        outfile_ticket = File.open("./output_prob/32_Tickets_#{sprintf('%02d',filecount)}.csv", "wb")
        outfile_comment = File.open("./output_prob/32_Ticket_Comments_#{sprintf('%02d',filecount)}.csv", "wb")
        outfile_user = File.open("./output_prob/32_Users_#{sprintf('%02d',filecount)}.csv", "wb")
        outfile_group = File.open("./output_prob/32_Groups_#{sprintf('%02d',filecount)}.csv", "wb")
      ...
      count += 1
      if (count % batch_size == 0) || (t.id == 252988)
        @logger.debug "Processed #{count} tickets."
        @logger.info "Created ./output_prob/32_Tickets_#{sprintf('%02d',filecount)}.csv"
        @logger.info "Created ./output_prob/32_Ticket_Comments_#{sprintf('%02d',filecount)}.csv"
        ...
        @logger.info "Created ./output_prob/32_Users_#{sprintf('%02d',filecount)}.csv"
        @logger.info "Created ./output_prob/32_Group_#{sprintf('%02d',filecount)}.csv"
